...
public class Login extends JFrame{
	...
	Login(){
		add(new JLabel("로그인") {{
			setBorder(createCompoundBorder(createMatteBorder(0, 0, 1, 0, Color.black), createEmptyBorder(10, 10, 0, 0)));
	...
	private void setLoginPanel() {
		...
		borderPanel.setBorder(createEmptyBorder(10,10,30,10));
	...
	private void setAction() {
		login.addActionListener(e->{
			...
			List<data> list = new Connections().select("select * from user where u_id = ? and u_pw = ?", i, p);
			if(list.isEmpty()) {
