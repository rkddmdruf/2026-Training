...
public class movieInfor extends JFrame{
	...
	movieInfor(int m_no, int u_no){
		...
		movie = c.select("SELECT movie.*, genre.g_name FROM moviedb.movie \r\n"
				+ "join genre on genre.g_no = movie.g_no \r\n"
				+ "where m_no = ?;", m_no).get(0);
		add(borderPanel);
		borderPanel.add(sc);
		new Z_setFrame(this, "영화 정보", 850, 450);
		setLogoPanel();
		setMainPanel();
		setAction();
		revalidate();
	...
	private void setMainPanel() {
		...
		예매판넬.setBorder(createEmptyBorder(0,0,20,0));
		예매판넬.add(예매);
		...
		List<data> review = c.select("SELECT review.*, u_birth, u_name, user.u_no FROM moviedb.review \r\n"
		...
		persent.setBorder(createEmptyBorder(50,0,50,0));
		...
		reCom.setBorder(createLineBorder(Color.black));
		for(int i = 0; i < review.size(); i++) {final int index = i;
			...
			com.setBorder(createLineBorder(Color.black));
			...
			com.add(new JPanel(new BorderLayout(5,5)) {{
				setBorder(createEmptyBorder(10,0,10,0));
