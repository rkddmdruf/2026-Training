...
public class movieSerch extends JFrame{
	...
	JComboBox<String> category = new JComboBox<String>(){{
		addItem("전체");
		for(List<Object> l : c.getData(2)) addItem(l.get(1).toString());
	}};
	...
	private void setMainPanel() {
		...
		switch (order.getSelectedIndex()) {
			case 0: {list = c.getData(3); break;}
			case 1: {list = c.getData(" SELECT movie.*, count(movie.m_no) as c FROM moviedb.reservation\r\n"
			...
			case 2: {list = c.getData("SELECT movie.*, avg(re_star) as a FROM moviedb.review \r\n"
