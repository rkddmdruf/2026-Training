...
public class movieChange extends JFrame{
	...
	JComboBox<String> category = new JComboBox<String>() {{
		for(List<Object> list : c.select(2)) addItem(list.get(1).toString());
	}};
	JComboBox<String> age = new JComboBox<String>() {{
		for(List<Object> list : c.select(4)) addItem(list.get(1).toString());
	}};
	JTextField name = new JTextField();
	JTextArea infor = new JTextArea() {{
		setBorder(createLineBorder(Color.black));
	}};
	TitledBorder inforBorder = new TitledBorder(createLineBorder(Color.black), "설명", TitledBorder.LEFT,TitledBorder.TOP);
	JScrollPane sc = new JScrollPane(infor) {{
		setBorder(createCompoundBorder(inforBorder, createEmptyBorder(5,2,2,2)));
	}};
	...
	movieChange(int m_no){
		this.m_no = m_no;
		list = c.select("select * from movie where m_no = ?", m_no);
	...
	private void setAction() {
		change.addActionListener(e->{
			...
			List<String> checkList = Arrays.asList(str);
			/*for(String s : str) {
				if(infor.getText().contains(s)) {
					JOptionPane.showMessageDialog(null, "욕설을 포함하고 있습니다.", "경고", JOptionPane.ERROR_MESSAGE);
					return;
				}
			}*/
			if(checkList.stream().anyMatch(infor.getText()::contains)) {
				JOptionPane.showMessageDialog(null, "욕설을 포함하고 있습니다.", "경고", JOptionPane.ERROR_MESSAGE);
				return;
			}//stream(잘모름)
			
			JOptionPane.showMessageDialog(null, "정보가 수정되었습니다.", "정보", JOptionPane.INFORMATION_MESSAGE);
			c.updateQuery("update movie set m_name = ? , l_no = ? , m_plot = ? , g_no = ? where m_no = ?;", name.getText(), age.getSelectedIndex(), infor.getText(), category.getSelectedIndex(), m_no);
			new movieSerch(-1);
			dispose();
			return;
		});
